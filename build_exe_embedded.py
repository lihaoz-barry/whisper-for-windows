...
def prepare_for_inno_setup(arch):
    ...
    with open(staging_dir / "Run Whisper Transcriber.bat", "w", encoding="utf-8") as f:
        ...
        f.write('        echo CUDA 12.6 detected, uninstalling all torch packages...\n')
        f.write('        python\\Scripts\\pip.exe uninstall -y torch torch-cpu torch-directml torch-tensorrt\n')
        f.write('        echo Installing compatible CUDA PyTorch...\n')
        f.write('        python\\Scripts\\pip.exe install -r requirements_cuda126.txt\n')
        f.write('    ) else if "!CUDA_VERSION!"=="12.4" (\n')
        f.write('        echo CUDA 12.4 detected, uninstalling all torch packages...\n')
        f.write('        python\\Scripts\\pip.exe uninstall -y torch torch-cpu torch-directml torch-tensorrt\n')
        f.write('        echo Installing compatible CUDA PyTorch...\n')
        f.write('        python\\Scripts\\pip.exe install -r requirements_cuda124.txt\n')
        f.write('    ) else if "!CUDA_VERSION!"=="11.8" (\n')
        f.write('        echo CUDA 11.8 detected, uninstalling all torch packages...\n')
        f.write('        python\\Scripts\\pip.exe uninstall -y torch torch-cpu torch-directml torch-tensorrt\n')
        f.write('        echo Installing compatible CUDA PyTorch...\n')
        f.write('        python\\Scripts\\pip.exe install -r requirements_cuda118.txt\n')
        f.write('    ) else (\n')
        f.write('        echo No compatible CUDA version detected, installing CPU-only PyTorch...\n')
        f.write('        python\\Scripts\\pip.exe install -r requirements_cpu.txt\n')
        f.write('    )\n')
        f.write('\n')
        f.write('set PYTHONPATH=%~dp0site-packages\n')
        ...
        f.write('    echo Checking for CUDA-capable GPU...\n')
        f.write('    python\\python.exe cuda_detector.py > cuda_version.txt\n')
        f.write('    set /p CUDA_VERSION_STR=<cuda_version.txt\n')
        f.write('    del cuda_version.txt\n')
        f.write('    echo Detected CUDA Version String: !CUDA_VERSION_STR!\n')
        f.write('\n')
        f.write('    set TORCH_REQ_FILE=requirements_cpu.txt\n')
        f.write('\n')
        f.write('    if "!CUDA_VERSION_STR!"=="NONE" (\n')
        f.write('        echo No CUDA detected or nvidia-smi failed. Using CPU PyTorch.\n')
        f.write('        set TORCH_REQ_FILE=requirements_cpu.txt\n')
        f.write('    ) else (\n')
        f.write('        echo CUDA detected: !CUDA_VERSION_STR! - Determining compatible PyTorch version...\n')
        f.write('        REM Parse Major.Minor version\n')
        f.write('        for /f "tokens=1,2 delims=." %%a in ("!CUDA_VERSION_STR!") do (\n')
        ...
        f.write('           echo ERROR: Could not parse CUDA version !CUDA_VERSION_STR!. Using CPU PyTorch.\n')
        f.write('           set TORCH_REQ_FILE=requirements_cpu.txt\n')
        f.write('        ) else if !CUDA_MAJOR! GEQ 13 (\n')
        f.write('            echo CUDA version !CUDA_VERSION_STR! >= 13.0. Using latest supported PyTorch (cu126).\n')
        f.write('            set TORCH_REQ_FILE=requirements_cuda126.txt\n')
        f.write('        ) else if !CUDA_MAJOR! EQU 12 (\n')
        f.write('            if !CUDA_MINOR! GEQ 6 (\n')
        f.write('                echo CUDA version !CUDA_VERSION_STR! >= 12.6. Using PyTorch cu126.\n')
        f.write('                set TORCH_REQ_FILE=requirements_cuda126.txt\n')
        f.write('            ) else if !CUDA_MINOR! GEQ 4 (\n')
        f.write('                echo CUDA version !CUDA_VERSION_STR! >= 12.4 and < 12.6. Using PyTorch cu124.\n')
        f.write('                set TORCH_REQ_FILE=requirements_cuda124.txt\n')
        f.write('            ) else (\n')
        f.write('                echo CUDA version !CUDA_VERSION_STR! >= 12.0 and < 12.4. Using PyTorch cu118.\n')
        f.write('                set TORCH_REQ_FILE=requirements_cuda118.txt\n')
        f.write('            )\n')
        f.write('        ) else if !CUDA_MAJOR! EQU 11 (\n')
        f.write('             if !CUDA_MINOR! GEQ 8 (\n')
        f.write('                 echo CUDA version !CUDA_VERSION_STR! >= 11.8 and < 12.0. Using PyTorch cu118.\n')
        f.write('                 set TORCH_REQ_FILE=requirements_cuda118.txt\n')
        f.write('             ) else (\n')
        f.write('                 echo CUDA version !CUDA_VERSION_STR! < 11.8. Using CPU PyTorch.\n')
        f.write('                 set TORCH_REQ_FILE=requirements_cpu.txt\n')
        f.write('             )\n')
        f.write('        ) else (\n')
        f.write('            echo CUDA version !CUDA_VERSION_STR! < 11.8. Using CPU PyTorch.\n')
        ...
        f.write('    echo Uninstalling any existing torch versions first (to avoid conflicts)...\n')
        ...
        f.write('\n')
        f.write('set PYTHONPATH=%~dp0site-packages\n')
        # ... rest of the batch file generation ...
