...
def prepare_for_inno_setup(arch):
    ...
    with open(staging_dir / "setup_embedded_python.bat", "w", encoding="utf-8") as f:
        ...
        f.write('python.exe -m pip install --upgrade setuptools wheel\n') # Changed from Scripts\pip.exe
        f.write('if %ERRORLEVEL% NEQ 0 (\n')
    ...
    with open(staging_dir / "Run Whisper Transcriber.bat", "w", encoding="utf-8") as f:
        ...
        f.write('cd "%~dp0"\n')
        f.write('\n')
        f.write(':: Define flag file names\n')
        f.write('set BASE_DEPS_FLAG=_base_deps_installed.flag\n')
        f.write('set WHISPER_FLAG=_whisper_installed.flag\n')
        f.write('set PYTORCH_CONFIGURED_FLAG=_pytorch_configured.flag\n') # New single flag
        f.write('\n')
        ...
        f.write(')\n')
        f.write('echo ================================================\n')
        f.write('echo Dependency Installation\n')
        f.write('echo ================================================\n')
        f.write('\n')
        ...
        f.write('    ) else (\n')
        f.write('        echo ERROR: Failed to install base dependencies from requirements_base.txt.\n')
        f.write('        pause\n')
        f.write('        exit /b 1\n')
        f.write('    )\n')
        f.write(') else (\n')
        f.write('    echo Base dependencies flag found. Skipping installation.\n')
        f.write(')\n')
        f.write('\n')
        f.write('echo Checking for PyTorch configuration flag: !PYTORCH_CONFIGURED_FLAG!\n')
        f.write('if exist "!PYTORCH_CONFIGURED_FLAG!" (\n')
        f.write('    echo PyTorch already configured. Skipping CUDA detection and PyTorch installation.\n')
        f.write('    GOTO PYTORCH_INSTALL_DONE\n')
        f.write(')\n')
        f.write('\n')
        f.write('echo PyTorch configuration flag not found. Proceeding with CUDA detection and PyTorch installation...\n')
        f.write('echo ================================================\n')
        ...
        f.write(') else if "!CUDA_MAJOR!" == "NONE" (\n') # Ensure this check is robust
        f.write('    echo No CUDA or parse error. Using CPU PyTorch.\n')
        f.write('    set TORCH_REQ_FILE=requirements_cpu.txt\n')
        ...
        f.write('    exit /b 1\n')
        f.write(')\n')
        f.write('echo PyTorch installed successfully from !TORCH_REQ_FILE!.\n')
        f.write('\n')
        f.write('echo Creating PyTorch configuration flag: !PYTORCH_CONFIGURED_FLAG!\n')
        f.write('echo. > "!PYTORCH_CONFIGURED_FLAG!"\n')
        f.write('echo PyTorch configuration complete and flag created.\n')
        f.write('\n')
        f.write(':PYTORCH_INSTALL_DONE\n')
        f.write('echo PyTorch section finished.\n')
        f.write('\n')
        f.write('echo Checking for Whisper installation flag: !WHISPER_FLAG!\n')
        f.write('\n')
        ...
        f.write('        echo openai-whisper installed successfully and flag created.\n')
        f.write('    ) else (\n')
        f.write('        echo ERROR: Failed to install openai-whisper.\n')
        f.write('        pause\n')
        f.write('        exit /b 1\n')
        f.write('    )\n')
        f.write(') else (\n')
        f.write('    echo Whisper installation flag found. Skipping installation.\n')
        f.write(')\n')
        f.write('\n')
        f.write('echo ================================================\n')
        f.write('set PYTHONPATH=%~dp0site-packages\n')
